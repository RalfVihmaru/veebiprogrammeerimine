<?php
  ...
  function storeuserprofile($description, $bgColor, $txtColor){
	...
	if($stmt->fetch()){
		...
		$stmt->close();
		$stmt = $conn->prepare("UPDATE vpuserprofiles3 SET description = ?; bgcolor = ?, txcolor = ? WHERE userid = ?");
		echo $conn->error;
		$stmt->bind_param("sssi", $description, $bgColor, $txtColor, $_SESSION["userID"]);
		if($stmt->execute()){
		$notice = "Proffiil edukalt uuendatud!";
		$_SESSION["bgColor"] = $bgColorFromDb;
	    $_SESSION["txtColor"] = $txtColorFromDb;
		} else {
			$notice = "profiili salvestamisel oli tõrge!" .$stmt->error;
		}
